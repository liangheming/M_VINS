...
bool IMUFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    if (jacobians)
    {
        ...
        Quatd corrected_delta_q = integration->delta_q * deltaQ(dq_dbg * (bgi - integration->linearized_bg));
        Mat3d ri_transpose = qi.inverse().toRotationMatrix();

        if (jacobians[0])
        {
            Eigen::Map<Eigen::Matrix<double, 15, 7, Eigen::RowMajor>> jacobian_pose_i(jacobians[0]);
            jacobian_pose_i.setZero();
            jacobian_pose_i.block<3, 3>(O_P, O_P) = -ri_transpose;
            jacobian_pose_i.block<3, 3>(O_P, O_R) = skewSymmetric(ri_transpose * (0.5 * g_vec * sum_dt * sum_dt + pj - pi - vi * sum_dt));

            jacobian_pose_i.block<3, 3>(O_R, O_R) = -(qj.inverse() * qi).toRotationMatrix();

            jacobian_pose_i.block<3, 3>(O_V, O_R) = skewSymmetric(ri_transpose * (g_vec * sum_dt + vj - vi));
        ...
        if (jacobians[1])
        {
            ...
            jacobian_speedbias_i.block<3, 3>(O_P, O_V - O_V) = -ri_transpose * sum_dt;
            jacobian_speedbias_i.block<3, 3>(O_P, O_BA - O_V) = -dp_dba;
            jacobian_speedbias_i.block<3, 3>(O_P, O_BG - O_V) = -dp_dbg;
            jacobian_speedbias_i.block<3, 3>(O_R, O_BG - O_V) = -Qleft(qj.inverse() * qi * integration->delta_q).bottomRightCorner<3, 3>() * dq_dbg;

            jacobian_speedbias_i.block<3, 3>(O_V, O_V - O_V) = -ri_transpose;
        ...
        if (jacobians[2])
        {
            ...
            jacobian_pose_j.block<3, 3>(O_P, O_P) = ri_transpose;
        ...
        if (jacobians[3])
        {
            ...
            jacobian_speedbias_j.block<3, 3>(O_V, O_V - O_V) = ri_transpose;
