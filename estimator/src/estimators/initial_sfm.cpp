...
bool GlobalSFM::construct(int frame_num, Quatd *qs, Vec3d *ts, int l,
                          ...
                          std::vector<SFMFeature> &sfm_f, std::map<int, Vec3d> &sfm_tracked_points)
{
    ...
    qs[l].z() = 0;
    ts[l].setZero();
    qs[frame_num - 1] = Quatd(relative_r);
    ...
    c_quats[l] = qs[l].inverse();
    c_rotations[l] = c_quats[l].toRotationMatrix();
    c_translations[l] = -1.0 * c_rotations[l] * ts[l];
    poses[l].block<3, 3>(0, 0) = c_rotations[l];
    poses[l].block<3, 1>(0, 3) = c_translations[l];

    c_quats[frame_num - 1] = qs[frame_num - 1].inverse();
    c_rotations[frame_num - 1] = c_quats[frame_num - 1].toRotationMatrix();
    ...
    for (int i = 0; i < frame_num; i++)
    {
        qs[i].w() = c_rotations_arr[i][0];
        qs[i].x() = c_rotations_arr[i][1];
        qs[i].y() = c_rotations_arr[i][2];
        qs[i].z() = c_rotations_arr[i][3];
        qs[i] = qs[i].inverse();
        ts[i] = -1.0 * (qs[i] * Vec3d(c_translations_arr[i][0], c_translations_arr[i][1], c_translations_arr[i][2]));
    }
